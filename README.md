...
Gemini API, Together AI, fal Seedance를 조합해 아이디어 입력부터 플롯, 컷 이미지, 컷 영상, 최종 mp4 병합까지 이어지는 숏폼 제작 스튜디오입니다.

## 핵심 파이프라인

- 플롯 기획: `gemini-2.5-flash` + Google Search 기반 IP 리서치 + structured JSON output
- 이미지 생성: `Together AI` 이미지 생성 API
- 영상 생성: `fal-ai/bytedance/seedance/v1/lite/image-to-video`
...
- 영상은 Seedance 1.0 Lite의 image-to-video 큐 API를 사용합니다.
...
FAL_VIDEO_MODEL=fal-ai/bytedance/seedance/v1/lite/image-to-video
...
- Seedance Lite는 `fal` 크레딧과 API 키가 있어야 호출할 수 있습니다.
